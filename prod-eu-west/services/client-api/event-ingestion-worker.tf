resource "aws_ecs_service" "event-ingestion-worker" {
  name            = "event-ingestion-worker"
  cluster         = data.aws_ecs_cluster.default.id
  launch_type     = "FARGATE"
  task_definition = aws_ecs_task_definition.event-ingestion-worker.arn
  desired_count   = 1
  ...
  service_registries {
    registry_arn = aws_service_discovery_service.event-ingestion-worker.arn
  }

}

resource "aws_cloudwatch_log_group" "event-ingestion-worker" {
  name = "/ecs/event-ingestion-worker"
}

resource "aws_ecs_task_definition" "event-ingestion-worker" {
  ...
  cpu                      = "2048"
  memory                   = "8192"
  container_definitions = templatefile("event-ingestion-worker.json",
    {
...
resource "aws_service_discovery_service" "event-ingestion-worker" {
  name = "event-ingestion-worker"
