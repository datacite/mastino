resource "aws_ecs_service" "queue-worker-stage" {
  name = "queue-worker-stage"
  cluster = data.aws_ecs_cluster.stage.id
  launch_type = "FARGATE"
  task_definition = aws_ecs_task_definition.queue-worker-stage.arn
  desired_count = 1
  ...
  service_registries {
    registry_arn = aws_service_discovery_service.queue-worker-stage.arn
  }

}

resource "aws_cloudwatch_log_group" "queue-worker-stage" {
  name = "/ecs/queue-worker-stage"
}

resource "aws_ecs_task_definition" "queue-worker-stage" {
  family = "queue-worker-stage"
...
resource "aws_service_discovery_service" "queue-worker-stage" {
  name = "queue-worker.stage"
